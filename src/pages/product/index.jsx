...
export class Product extends Component {
    ...
    this.columns = [
      {
        ...
        key: 'image',
        render: (text, record) => {
          return <img style={{ width: 100, height: 100 }} src={this.imgIp + record.image.split(',')[0]} />;
        },
      },
      {
        title: '标题',
        ...
        render: text => {
          return (
            <div className="long-title" title={text}>
              {text}
            </div>
          );
        },
            ...
            <div style={{ cursor: 'pointer' }} className="all-operate">
              <span
                onClick={() => {
                  ...
                  this.getProductSkuListPage(record.id), this.updateProduct(record);
                  this.getbrandList();
  ...
  updateProduct = record => {
    ...
    const { startTime, detailUrl, image, endTime, title, price, productCatId, productBannerId } = record;
  ...
  saveProduct = () => {
    ...
    form.validateFields((err, values) => {
      if (!err) {
        ...
        (values.detail = richText), (values.image = image);
